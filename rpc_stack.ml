...
type orpie_data_t = | RpcInt of Big_int.big_int
                    ...
                    | RpcComplexUnit of Gsl_complex.complex * Units.unit_set_t
                    | RpcFloatMatrixUnit of Matrix.matrix * Units.unit_set_t
                    | RpcComplexMatrixUnit of Matrix_complex.matrix *
                                              Units.unit_set_t
...
type stack_data_t = | StackInt of Big_int.big_int * stack_int_string_t
                    ...
                    | StackGsl_complexUnit of Gsl_complex.complex * Units.unit_set_t * stack_cmpx_unit_string_t
                    | StackFloatMatrixUnit of Matrix.matrix * Units.unit_set_t *
                                              stack_fmat_unit_string_t
                    | StackGsl_complexMatrixUnit of Matrix_complex.matrix *
...
let orpie_data_of_stack_data (sd : stack_data_t) =
   ...
   |StackGsl_complexUnit (cc, uu, _)       -> RpcComplexUnit (cc, uu)
   |StackFloatMatrixUnit (fm, uu, _)   -> RpcFloatMatrixUnit (fm, uu)
   |StackGsl_complexMatrixUnit (cm, uu, _) -> RpcComplexMatrixUnit (cm, uu)
...
let stack_data_of_orpie_data (od : orpie_data_t) =
   ...
   |RpcComplexMatrixUnit (cm, uu) ->
      StackGsl_complexMatrixUnit (cm, uu,
         {cmat_rect_line    = None;
...
let c_of_f ff = {Gsl_complex.re = ff; Gsl_complex.im = 0.0}
   ...
   object(self)
      ...
      method private lookup_or_create_string disp_mode calc_modes index =
         ...
         let lookup_result =
            ...
            |StackGsl_complexUnit (cc, uu, cc_str) ->
            ...
            |StackGsl_complexMatrixUnit (cm, uu, cm_str) ->
      ...
      method private create_cmpx_unit_string calc_modes cc uu cc_str =
         ...
         |Rect ->
            let s = append_units 
            (sprintf "(%.15g, %.15g)" cc.Gsl_complex.re cc.Gsl_complex.im) in
            if conserve_memory then () 
            else cc_str.c_rect <- Some s;
            s
         |Polar ->
            let r = sqrt (cc.Gsl_complex.re *. cc.Gsl_complex.re +.
               cc.Gsl_complex.im *. cc.Gsl_complex.im)
            and theta = atan2 cc.Gsl_complex.im cc.Gsl_complex.re in
            begin match calc_modes.angle with
            |Rad ->
      ...
      method private create_fmat_unit_string disp_mode fm uu fm_str =
         ...
         |Line ->
            let s = 
               ...
               let rows, cols = (Matrix.dims fm) in
         ...
         |Line ->
            let s = 
               (* looks like [[ (a11re, a11im), (a12re, a12im) ][ (a21re,
                  a21im), (a22re, a22im) ] *)
               let rows, cols = (Matrix_complex.dims cm) in
               let initial_string = "[" in
               let line = ref initial_string in
               for n = 0 to rows - 1 do
                  line := !line ^ "[ ";
                  for m = 0 to cols - 2 do
                     ...
                     |Rect ->
                        line := !line ^ (sprintf "(%.15g, %.15g), " 
                           cm.{n, m}.Gsl_complex.re cm.{n, m}.Gsl_complex.im)
                     |Polar ->
                        let rr = cm.{n, m}.Gsl_complex.re
                        and ii = cm.{n, m}.Gsl_complex.im in
                  ...
                  |Rect ->
                     line := !line ^ (sprintf "(%.15g, %.15g) ]" 
                        cm.{n, cols-1}.Gsl_complex.re cm.{n, cols-1}.Gsl_complex.im)
                  |Polar ->
                     let rr = cm.{n, cols-1}.Gsl_complex.re
                     and ii = cm.{n, cols-1}.Gsl_complex.im in
                     let r = sqrt (rr *. rr +. ii *. ii)
                     and theta = atan2 ii rr in
                     begin match calc_modes.angle with
                     |Rad ->
            ...
            let s =
               ...
               for m = 0 to pred cols do
                  for n = 0 to pred rows do
                     match calc_modes.complex with
                     |Rect ->
                        let dummy_re = sprintf "%-.15g" cm.{n, m}.Gsl_complex.re in
                        let dr_len = String.length dummy_re in
                        if dr_len > max_width.(m).(0) then
                           max_width.(m).(0) <- dr_len
                        else
                           ();
                        let dummy_im = sprintf "%-.15g" cm.{n, m}.Gsl_complex.im in
               ...
               for n = 0 to rows - 1 do
                  ...
                  for m = 0 to cols - 2 do
                     ...
                     |Rect ->
                        line := !line ^ (sprintf "(%*.15g, %*.15g), " 
                           max_width.(m).(0) cm.{n, m}.Gsl_complex.re 
                           max_width.(m).(1) cm.{n, m}.Gsl_complex.im)
                     |Polar ->
                        begin
                           let rr = cm.{n, m}.Gsl_complex.re
                           and ii = cm.{n, m}.Gsl_complex.im in
                  ...
                  |Rect ->
                     line := !line ^ (sprintf "(%*.15g, %*.15g) ]" 
                        max_width.(cols-1).(0) cm.{n, cols-1}.Gsl_complex.re 
                        max_width.(cols-1).(1) cm.{n, cols-1}.Gsl_complex.im)
                  |Polar ->
                     begin
                        let rr = cm.{n, cols-1}.Gsl_complex.re
                        and ii = cm.{n, cols-1}.Gsl_complex.im in
