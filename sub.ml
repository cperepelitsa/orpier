   ...
   |RpcInt el1 -> (
      ...
      |RpcComplexUnit (el2, uu2) ->
         if uu2 <> Units.empty_unit then begin
            ...
            let (c, u) = cunit_of_cpx (Gsl_complex.sub c_el1 el2) in
   ...
   |RpcFloatUnit (el1, uu1) -> (
      ...
      |RpcComplexUnit (el2, uu2) ->
         begin try
            ...
            in
            stack#push (RpcComplexUnit (Gsl_complex.sub c_el1 el2, uu2))
         with Units.Units_error s -> 
            stack#push gen_el1;
            stack#push gen_el2;
            raise_invalid s
   ...
   |RpcComplexUnit (el1, uu1) -> (
      ...
      |RpcInt el2 ->
         if uu1 <> Units.empty_unit then begin
            ...
            let (c, u) = cunit_of_cpx (Gsl_complex.sub el1 c_el2) in
            stack#push (RpcComplexUnit (c, u))
      |RpcFloatUnit (el2, uu2) ->
         begin try
            let conv = c_of_f (Units.conversion_factor uu1 uu2 !Rcfile.unit_table) in
            let c_el1 = Gsl_complex.mul el1 conv in
            let c_el2 = c_of_f el2 in
            stack#push (RpcComplexUnit (Gsl_complex.sub c_el1 c_el2, uu2))
         with Units.Units_error s ->
            stack#push gen_el1;
            stack#push gen_el2;
            raise_invalid s
      ...
      |RpcFloatMatrixUnit (el2, u2) ->
         ...
         if dim1 = dim2 then
            try
               ...
               Matrix_complex.sub result c_el2;
