            ...
            |true ->
               ...
               and theta = 
                  ...
                  |Deg ->
                     ...
                     buffer.im_exponent))
               in
               let cc = {re = r *. (Pervasives.cos theta); im = r *. (Pervasives.sin theta)}
               and uu = Units.units_of_string iface.units_entry_buffer
                  !Rcfile.unit_table
               in
               RpcComplexUnit (cc, uu)
               ...
               |true ->
                  ...
                  and theta =
                     ...
                     |Deg ->
                        ...
                        iface.gen_buffer.(i).im_exponent))
                  in
                  temp_arr.(i) <- {re = r *. (Pervasives.cos theta); 
                  im = r *. (Pervasives.sin theta)}
               end
            done;
            let uu = Units.units_of_string iface.units_entry_buffer
               !Rcfile.unit_table 
            in
            RpcComplexMatrixUnit (Matrix_complex.of_array temp_arr
...
let handle_begin_complex (iface : interface_state_t) =
   ...
   match iface.entry_type with
   |FloatEntry ->
      (iface.entry_type <- Gsl_complexEntry;
      draw_update_entry iface)
   |FloatMatrixEntry ->
      (iface.entry_type <- Gsl_complexMatrixEntry;
      draw_update_entry iface)
   |_ ->
      ()
...
let handle_scientific_notation (iface : interface_state_t) =
   ...
   |FloatEntry | FloatMatrixEntry ->
      if String.length iface.gen_buffer.(iface.curr_buf).re_mantissa > 0 then
         ...
         ()
   |Gsl_complexEntry | Gsl_complexMatrixEntry ->
      if iface.is_entering_imag then
...
let handle_begin_units (iface : interface_state_t) =
   ...
   |Gsl_complexEntry ->
      ...
      iface.is_entering_units <- true;
      draw_update_entry iface
   |FloatMatrixEntry | Gsl_complexMatrixEntry ->
      iface.has_entry         <- true;
      iface.interface_mode    <- UnitEditMode;
      iface.is_entering_units <- true;
      draw_update_entry iface
