...
let add (stack : rpc_stack) (evaln : int -> unit) =
   ...
   |RpcInt el1 -> (
      ...
      |RpcComplexUnit (el2, uu2) ->
         if uu2 <> Units.empty_unit then begin
            ...
            let (c, u) = cunit_of_cpx (Gsl_complex.add c_el1 el2) in
   ...
   |RpcFloatUnit (el1, uu1) -> (
      ...
      |RpcComplexUnit (el2, uu2) ->
         begin try
            ...
            stack#push (RpcComplexUnit (Gsl_complex.add c_el1 el2, uu2))
         with Units.Units_error s ->
            stack#push gen_el1;
            stack#push gen_el2;
            raise_invalid s
   ...
   |RpcComplexUnit (el1, uu1) -> (
      ...
      |RpcInt el2 ->
         if uu1 <> Units.empty_unit then begin
            ...
            let (c, u) = cunit_of_cpx (Gsl_complex.add el1 c_el2) in
            stack#push (RpcComplexUnit (c, u))
      |RpcFloatUnit (el2, uu2) ->
         begin try
            let conv = c_of_f (Units.conversion_factor uu1 uu2 !Rcfile.unit_table) in
            let c_el1 = Gsl_complex.mul conv el1 in
            let c_el2 = c_of_f el2 in
            stack#push (RpcComplexUnit (Gsl_complex.add c_el1 c_el2, uu2))
         with Units.Units_error s ->
            stack#push gen_el1;
            stack#push gen_el2;
            raise_invalid s
   ...
   |RpcComplexMatrixUnit (el1, u1) -> (
      match gen_el2 with
      |RpcFloatMatrixUnit (el2, u2) ->
         let dim1 = (Matrix_complex.dims el1) and
         dim2     = (Matrix.dims el2) in
         if dim1 = dim2 then
            try
               let conv = c_of_f (Units.conversion_factor u1 u2 !Rcfile.unit_table) in
               let c_el2 = cmat_of_fmat el2 in
               let copy = Matrix_complex.copy el1 in
               Matrix_complex.scale copy conv;
               Matrix_complex.add copy c_el2;
