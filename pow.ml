...
let pow (stack : rpc_stack) (evaln : int -> unit) =
   ...
   |RpcInt el1 -> (
      ...
      |RpcFloatUnit (el2, uu2) ->
         if uu2 <> Units.empty_unit then begin
            ...
            if f_el1 >= 0.0 || f_el2 = float_of_int (int_of_float f_el2) then
               ...
               let (c, u) = cunit_of_cpx (Gsl_complex.pow c_el1 c_el2) in
      ...
      |RpcComplexUnit (el2, uu2) ->
         if uu2 <> Units.empty_unit then begin
            ...
            let (c, u) = cunit_of_cpx (Gsl_complex.pow c_el1 el2) in
            stack#push (RpcComplexUnit (c, u))
      |_ ->
         (stack#push gen_el1;
         stack#push gen_el2;
   ...
   |RpcFloatUnit (el1, uu1) -> (
      ...
      |RpcFloatUnit (el2, uu2) ->
         if uu2 <> Units.empty_unit then begin
            ...
            let c_prod = Gsl_complex.pow c_el1 c_el2 in
            if c_prod.Gsl_complex.im <> 0.0 then
               stack#push (RpcComplexUnit (c_prod, Units.pow uu1 el2))
            else
               stack#push (RpcFloatUnit (c_prod.Gsl_complex.re, Units.pow uu1 el2))
      |RpcComplexUnit (el2, uu2) ->
         if uu2 <> Units.empty_unit then begin
            stack#push gen_el1;
            stack#push gen_el2;
   ...
   |RpcComplexUnit (el1, uu1) -> (
      ...
      |RpcInt el2 ->
         ...
         let c_prod = Gsl_complex.pow el1 c_el2 in
      ...
      |RpcComplexUnit (el2, uu2) ->
         if uu2 <> Units.empty_unit then begin
            ...
            let (c, u) = cunit_of_cpx (Gsl_complex.pow el1 el2) in
            stack#push (RpcComplexUnit (c, u))
      |_ ->
         (stack#push gen_el1;
         stack#push gen_el2;
