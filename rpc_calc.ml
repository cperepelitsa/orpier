   ...
   object(self)
      ...
      method private internal_neg () =
         ...
         |RpcComplexUnit (el, uu) ->
            stack#push (RpcComplexUnit (Gsl_complex.negative el, uu))
         |RpcFloatMatrixUnit (el, uu) ->
            let copy = Matrix.copy el in
            (Matrix.scale copy (-1.0);
            stack#push (RpcFloatMatrixUnit (copy, uu)))
         |RpcComplexMatrixUnit (el, uu) ->
            let copy = Matrix_complex.copy el in
            (Matrix_complex.scale copy {Gsl_complex.re=(-1.0); Gsl_complex.im=0.0};
      ...
      method private internal_sq () =
         ...
         |RpcComplexUnit (el, uu) ->
            stack#push (RpcComplexUnit (Gsl_complex.mul el el, Units.pow uu 2.))
         |RpcFloatMatrixUnit (el, uu) ->
            let n, m = (Matrix.dims el) in
            if n = m then
               let result = Matrix.create n m in
               (Blas.gemm Blas.NoTrans Blas.NoTrans 1.0 el el 0.0 result;
         ...
         |RpcComplexMatrixUnit (el, uu) ->
            ...
            if m = n then
               let result = Matrix_complex.create n m in
               Blas.Complex.gemm Blas.NoTrans Blas.NoTrans
               complex_one el el complex_zero result;
      ...
      method private internal_sqrt () =
         ...
         |RpcFloatUnit (el, uu) ->
            if el < 0.0 then
               let cc = c_of_f el in
               stack#push (RpcComplexUnit (Gsl_complex.sqrt cc, Units.pow uu 0.5))
            else
               stack#push (RpcFloatUnit (sqrt el, Units.pow uu 0.5))
         |RpcComplexUnit (el, uu) ->
            stack#push (RpcComplexUnit (Gsl_complex.sqrt el, Units.pow uu 0.5))
         |RpcVariable s ->
            stack#push gen_el;
            let err_msg = 
               Printf.sprintf "variable \"%s\" has not been evaluated" s 
      ...
      method private internal_ten_pow_x () =
         ...
         |RpcComplexUnit (el, uu) ->
            if uu <> Units.empty_unit then begin
               ...
               let (c, u) = cunit_of_cpx (Gsl_complex.pow (cmpx_of_float 10.0) el) in
               stack#push (RpcComplexUnit (c, u))
         |RpcVariable s ->
            stack#push gen_el;
            let err_msg = 
      ...
      method private internal_conj () =
         ...
         |RpcComplexMatrixUnit (el, uu) ->
            ...
            let rows, cols = Matrix_complex.dims el and
            arr = Matrix_complex.to_array el in
            let conj_arr = Array.map Gsl_complex.conjugate arr in
            let conj_mat = Matrix_complex.of_array conj_arr rows cols in
      ...
      method private internal_asinh () =
         ...
         match gen_el with
         |RpcInt el ->
            let (f, u) = funit_of_float (Math.asinh (float_of_big_int el)) in
            stack#push (RpcFloatUnit (f, u))
         |RpcFloatUnit (el, uu) ->
            if uu <> Units.empty_unit then begin
               stack#push gen_el;
               ...
               let (f, u) = funit_of_float (Math.asinh el) in
               stack#push (RpcFloatUnit (f, u))
         |RpcComplexUnit (el, uu) ->
            if uu <> Units.empty_unit then begin
               stack#push gen_el;
      ...
      method private internal_acosh () =
         ...
         match gen_el with
         |RpcInt el ->
            let (f, u) = funit_of_float (Math.acosh (float_of_big_int el)) in
            stack#push (RpcFloatUnit (f, u))
         |RpcFloatUnit (el, uu) ->
            if uu <> Units.empty_unit then begin
               stack#push gen_el;
               ...
               let (f, u) = funit_of_float (Math.acosh el) in
               stack#push (RpcFloatUnit (f, u))
         |RpcComplexUnit (el, uu) ->
            if uu <> Units.empty_unit then begin
               stack#push gen_el;
      ...
      method private internal_atanh () =
         ...
         match gen_el with
         |RpcInt el ->
            let (f, u) = funit_of_float (Math.atanh (float_of_big_int el)) in
            stack#push (RpcFloatUnit (f, u))
         |RpcFloatUnit (el, uu) ->
            if uu <> Units.empty_unit then begin
               stack#push gen_el;
               ...
               let (f, u) = funit_of_float (Math.atanh el) in
               stack#push (RpcFloatUnit (f, u))
         |RpcComplexUnit (el, uu) ->
            if uu <> Units.empty_unit then begin
               stack#push gen_el;
      ...
      method private internal_re () =
         ...
         |RpcFloatUnit (el, uu) ->
            stack#push gen_el
         |RpcComplexUnit (el, uu) ->
            stack#push (RpcFloatUnit (el.Gsl_complex.re, uu))
         |RpcFloatMatrixUnit (el, uu) ->
            stack#push gen_el
         |RpcComplexMatrixUnit (el, uu) ->
            let n, m = Matrix_complex.dims el
            and carr = Matrix_complex.to_array el in
            let farr = Array.make (n * m) 0.0 in
            for i = 0 to pred (n * m) do
               farr.(i) <- carr.(i).Gsl_complex.re
            done;
            stack#push (RpcFloatMatrixUnit (Matrix.of_array farr n m, uu))
         |RpcVariable s ->
            stack#push gen_el;
            let err_msg = 
               Printf.sprintf "variable \"%s\" has not been evaluated" s 
      ...
      method private internal_im () =
         ...
         |RpcComplexUnit (el, uu) ->
            stack#push (RpcFloatUnit (el.Gsl_complex.im, uu))
         |RpcFloatMatrixUnit (el, uu) ->
            let n, m = Matrix.dims el in
            let farr = Array.make (n * m) 0.0 in
            stack#push (RpcFloatMatrixUnit (Matrix.of_array farr n m, uu))
      ...
      method private internal_gamma () =
         ...
         |RpcInt el ->
            begin try
               let (f, u) = funit_of_float (Sf.gamma (float_of_big_int el)) in
               stack#push (RpcFloatUnit (f, u))
            with
               Error.Gsl_exn (err, errstr) ->
                  (stack#push gen_el;
         ...
         |RpcFloatUnit (el, uu) ->
            if uu <> Units.empty_unit then begin
               ...
               begin try
                  let (f, u) = funit_of_float (Sf.gamma el) in
                  stack#push (RpcFloatUnit (f, u))
               with
                  Error.Gsl_exn (err, errstr) ->
                     (stack#push gen_el;
      ...
      method private internal_lngamma () =
         ...
         |RpcInt el ->
            begin try
               let (f, u) = funit_of_float (Sf.lngamma (float_of_big_int el)) in
               stack#push (RpcFloatUnit (f, u))
            with
               Error.Gsl_exn (err, errstr) ->
                  (stack#push gen_el;
         ...
         |RpcFloatUnit (el, uu) ->
            if uu <> Units.empty_unit then begin
               ...
               begin try
                  let (f, u) = funit_of_float (Sf.lngamma el) in
                  stack#push (RpcFloatUnit (f, u))
               with
                  Error.Gsl_exn (err, errstr) ->
                     (stack#push gen_el;
      ...
      method private internal_erf () =
         ...
         |RpcInt el ->
            begin try
               let (f, u) = funit_of_float (Sf.erf (float_of_big_int el)) in
               stack#push (RpcFloatUnit (f, u))
            with
               Error.Gsl_exn (err, errstr) ->
                  (stack#push gen_el;
         ...
         |RpcFloatUnit (el, uu) ->
            if uu <> Units.empty_unit then begin
               ...
               begin try
                  let (f, u) = funit_of_float (Sf.erf el) in
                  stack#push (RpcFloatUnit (f, u))
               with
                  Error.Gsl_exn (err, errstr) ->
                     (stack#push gen_el;
      ...
      method private internal_erfc () =
         ...
         |RpcInt el ->
            begin try
               let (f, u) = funit_of_float (Sf.erfc (float_of_big_int el)) in
               stack#push (RpcFloatUnit (f, u))
            with
               Error.Gsl_exn (err, errstr) ->
                  (stack#push gen_el;
         ...
         |RpcFloatUnit (el, uu) ->
            if uu <> Units.empty_unit then begin
               ...
               begin try
                  let (f, u) = funit_of_float (Sf.erfc el) in
                  stack#push (RpcFloatUnit (f, u))
               with
                  Error.Gsl_exn (err, errstr) ->
                     (stack#push gen_el;
            ...
            if stack#length > 0 then begin
               ...
               |RpcFloatUnit (el, uu) ->
                  if uu <> Units.empty_unit then begin
                     ...
                     begin try
                        let (f, u) = funit_of_float (Sf.gamma (el +. 1.0)) in
                        stack#push (RpcFloatUnit (f, u));
                        true
                     with
                        Error.Gsl_exn (err, errstr) ->
      ...
      method private internal_transpose () =
         ...
         |RpcFloatMatrixUnit (el, uu) ->
            let n, m = (Matrix.dims el) in
            let trans_mat = Matrix.create m n in
            Matrix.transpose trans_mat el;
            stack#push (RpcFloatMatrixUnit (trans_mat, uu))
         |RpcComplexMatrixUnit (el, uu) ->
            let n, m = (Matrix_complex.dims el) in
            let trans_mat = Matrix_complex.create m n in
            Matrix_complex.transpose trans_mat el;
               ...
               |RpcFloatUnit (el1, uu1) ->
                  ...
                  |RpcFloatUnit (el2, uu2) ->
                     if uu1 <> Units.empty_unit || uu2 <> Units.empty_unit then begin
                        ...
                        begin try
                           let log_coeff = (Sf.lngamma (el1 +. 1.0)) -.
                           (Sf.lngamma (el2 +. 1.0)) -. 
                           (Sf.lngamma (el1 -. el2 +. 1.0)) in
               ...
               |RpcFloatUnit (el1, uu1) ->
                  ...
                  |RpcFloatUnit (el2, uu2) ->
                     if uu1 <> Units.empty_unit || uu2 <> Units.empty_unit then begin
                        ...
                        begin try
                           let log_perm = (Sf.lngamma (el1 +. 1.0)) -.
                           (Sf.lngamma (el1 -. el2 +. 1.0)) in
      ...
      method private internal_total () =
         ...
         |RpcFloatMatrixUnit (mat, uu) ->
            ...
            let ones = Matrix.of_array ones_arr 1 n in
            let result = Matrix.create 1 m in
            Blas.gemm Blas.NoTrans Blas.NoTrans 1.0 ones mat
            0.0 result;
      ...
      method private internal_standardize_units () =
         ...
         |RpcComplexUnit (el, uu) ->
            ...
            stack#push (RpcComplexUnit 
               (Gsl_complex.mul el c_coeff, std.Units.comp_units))
         |RpcFloatMatrixUnit (el, uu) ->
            let std = get_std uu in
            let result = Matrix.copy el in
            Matrix.scale result std.Units.coeff;
            stack#push (RpcFloatMatrixUnit 
            (result, std.Units.comp_units))
         |RpcComplexMatrixUnit (el, uu) ->
            let std = get_std uu in
            let c_coeff = c_of_f std.Units.coeff in
            let result = Matrix_complex.copy el in
            Matrix_complex.scale result c_coeff;
      ...
      method private internal_convert_units () =
         ...
         |RpcFloatUnit (el2, uu2) ->
            ...
            |RpcComplexUnit (el1, uu1) ->
               begin try
                  let c_conv = {
                     Gsl_complex.re = Units.conversion_factor uu1 uu2 !Rcfile.unit_table;
                     Gsl_complex.im = 0.0
                  } in
                  stack#push (RpcComplexUnit (Gsl_complex.mul el1 c_conv, uu2))
               with Units.Units_error s ->
                  stack#push gen_el1;
                  stack#push gen_el2;
                  raise_invalid s
            ...
            |RpcComplexMatrixUnit (el1, uu1) ->
               begin try
                  ...
                  Matrix_complex.scale result conv;
      ...
      method private internal_trace () =
         ...
         |RpcComplexMatrixUnit (el, uu) ->
            let n, m = Matrix_complex.dims el in
            if n = m then begin
               let result = ref complex_zero in
               for i = 0 to pred n do
                  result := Gsl_complex.add !result el.{i, i}
               done;
